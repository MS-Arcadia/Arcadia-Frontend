 ...
 *
 * Prices are typed as digit strings, the same convention `ReviewCard` uses for
 * a suggested price: the form never touches a JS number, it converts the typed
 * major-unit digits to a minor-unit string with `BigInt` right before the
 * request goes out — see `toMinorUnits` below.
 */
