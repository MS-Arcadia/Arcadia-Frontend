...
/**
 * Four ways to acquire one game, in one place — including gifting a title
 * already in the caller's library. Ownership is checked on the recipient.
 *
...
export function AcquirePanel({ game, owned, defaultIntent }: Props) {
  ...
  const giftOnly = owned && !preorder
  const defaultTab = giftOnly || defaultIntent === "gift" ? "gift" : "buy"
    ...
    <div className="rounded-xl border border-border bg-card p-5">
      ...
      {owned && preorder ? (
        ...
        <Tabs defaultValue={defaultTab}>
          ...
          {!owned && !preorder && !free && (
            <TabsContent value="instalments" className="space-y-4 pt-5">
              ...
              <p className="text-xs leading-relaxed text-muted-foreground">
                ...
                one. Miss a payment for longer than the grace period and the
                game is removed — what you have already paid is not returned.
              </p>
          ...
          <TabsContent
            value="gift"
            className={cn("space-y-4", !giftOnly && "pt-5")}
          >
            <div className="flex items-baseline justify-between gap-3">
              <span className="text-xs text-muted-foreground">
                Charged to your wallet
